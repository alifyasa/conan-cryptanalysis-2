...
def get_all_messages(
    recv_socket: socket.socket, stop_str="Jawaban = ", print_message=False
):
    msg = ""
    while True:
        msg_part = recv_socket.recv(1024).decode()
        if msg_part and print_message:
            print(msg_part)
        msg += msg_part
...
def get_challenge(recv_socket: socket.socket):
    ...
    paket_soal, rsa_n, rsa_e, ciphertext = re.search(
        CHALLENGE_PATTERN, question
    ).groups()
